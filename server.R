...
genre.recommendation <- read.csv('data/Recommendation_Highest_Rating.csv')
genre.recommendation.popular <- read.csv('data/Recommendation_PopularityBased.csv')
...
shinyServer(function(input, output, session) {
    ...
    lapply(1:num_rows, function(i) {
      list(fluidRow(lapply(1:num_books, function(j) {
        list(box(width = 2,
                 ...
                 #div(style = "text-align:center", img(src = 'pic.jpg', style = "max-height:150")),
  ...
  df1 <- eventReactive(input$btn1, {
      genre1 <- genre.recommendation[,input$genre1]
      print(genre1)
  }) 
  
  df2 <- eventReactive(input$btn2, {
    genre1 <- genre.recommendation.popular[,input$genre1]
    print(genre1)
  }) # clicked on button
  ...
  output$results2 <- renderUI({
    ...
    fluidRow(
      ...
      box(
        ...
        renderTable(recom_result, colnames = FALSE)
        ))})
